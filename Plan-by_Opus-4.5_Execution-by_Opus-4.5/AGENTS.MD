...
### 4.4 Handling ESLint 9 Circular Reference
If `pnpm run lint` fails with `TypeError: Converting circular structure to JSON`, this is a known issue with `FlatCompat` and some Next.js plugins in strict mode.
* **Workaround**: Simplify `eslint.config.mjs` to only extend `next/core-web-vitals` temporarily or ensure `eslint-plugin-react` is not conflicting.
* **Critical**: Do not let this block the build if code validity is confirmed via `pnpm run build`.
...
### 7.3 Tailwind 4 Configuration
For Tailwind 4, `postcss.config.mjs` must be:
```js
const config = {
  plugins: {
    '@tailwindcss/postcss': {},
...
```
Do not use `tailwindcss` directly as a plugin.
...
### 10.5 “Cannot find module” or “Module resolution failed”
* **Solution**: Stale cache or phantom dependencies.
* Run: `rm -rf node_modules .next pnpm-lock.yaml && pnpm install`
* This often fixes `Cannot find module '@tailwindcss/postcss'` and similar Turbopack/pnpm issues.
